...
## S0 — Setup & Data Foundation · `Status: 🟡 In progress`
...
- [ ] S0-2 `fixtures/` standard: fictional company/operator universe definition (names, products) + fixture naming rules — a checklist that audits similarity-to-real-data risk against SECURITY.md
- [ ] S0-3 3–5 synthetic Turkish BRDs (`.docx`): at different maturity levels (clean templated / messy / table-free), telco scenarios (campaign rule, billing change, CRM flow, integration)
- [ ] S0-4 Ledger schema v0 (as a document): `BRD → work item → effort given (three-point?) → actuals (if any) → meta (team, domain, date)`; seed-set field mapping table (xlsx/csv columns → schema)
- [ ] S0-5 Seed set inventory template: the checklist the project owner uses to compile the internal archive (past BRDs + effort tables) IN-HOUSE — only the template enters the repo, never the data
- [ ] S0-6 Golden set design note: evaluation scenarios, reference decompositions and metric definitions (`evals/README.md`)
...
- [ ] S1-1 Python tooling: uv workspace, ruff, mypy, pytest, pre-commit; `packages/` + `apps/` layout (AGENTS.md §5)
...
- [ ] S9-2 Git sync service: multi-repo clone/fetch, index refresh triggers
